...
export const findAllApproved = async () => {
  ...
  const { rows } = await pool.query(
    `SELECT p.id, p.user_id, p.title, p.content, p.status, p.created_at, p.updated_at, p.image_path AS "imagePath", u.username 
    FROM posts p JOIN users u ON p.user_id = u.id WHERE p.status = 'approved' ORDER BY p.created_at DESC`
  );
  return rows;
};
...
export const create = async (userId, title, content, imagePath) => {
  const { rows } = await pool.query(
